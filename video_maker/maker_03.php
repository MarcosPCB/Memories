<?php
    ...
    if(isset($_GET['i']) && isset($_GET['a']) && isset($_GET['d']) && isset($_GET['i0']) && isset($_GET['i1']) && isset($_GET['i2']) && isset($_GET['i3']))
    {
        ...
        exec('./ffmpeg -y -i "'.'./temp/'.$id.'_f3.mp4" -i "_assets/p5.mov" -c:v libx264 -crf 5 -framerate 25 -r 25 -shortest -filter_complex "[0][1]overlay[out]" -map "[out]" "'.'./temp/'.$id.'_f4_v.mp4"', $o, $r);
        ...
        //em caso de falha, ele apaga os assets e gera o erro
         if($r == 0 && file_exists('./temp/'.$id.'_f4_v.mp4') == true)
            echo('<script>$("#LOADING_BAR").width("36%");$("#LOADING_BAR")[0].ariaValueNow = 36; $("#LOADING_BAR")[0].innerText = "36%"</script>');
        else
        {
            ...
            $str = bin2hex($o);
            echo('<script>$ ("#ERRORPHP").click(function() { window.location = "error.php?e=7.1&r='.$r.'&o='.$str.'"; }); </script>');
            echo('<script>$("#ERRORPHP").trigger("click");</script>'); 
            
            die();
         }
        
        echo('<script>$("#LOADPHP_04").trigger("click");</script>'); //Próxima etapa
        die();
    }
?>
