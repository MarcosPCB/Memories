<?php
    ...
    if(isset($_GET['i']) && isset($_GET['a']) && isset($_GET['d']) && isset($_GET['i0']) && isset($_GET['i1']) && isset($_GET['i2']) && isset($_GET['i3']))
    {
        ...
        //Aqui ele insere a primeira imagem
        ...
        exec('./ffmpeg -y -i "_assets/BG_PNG_01.mp4" -i "'.$_GET['i0'].'" -i "_assets/p4.mp4" -c:v libx264 -crf 5 -framerate 25 -r 25 -shortest -filter_complex "[1]scale=740:740:force_original_aspect_ratio=decrease,pad=760:760:(ow-iw)/2:(oh-ih)/2,setsar=1[im1];[0][im1]overlay=x=180:y=110,rotate=-1.9*PI/180,trim=duration=3[ovl];[2]chromakey=0x00FF00:0.2:0.1:0[ckout];[ovl][ckout]overlay[out]" -map "[out]" "'.'./temp/'.$id.'_f3.mp4" 2>&1', $o, $r);
        ...
        if($r == 0 && file_exists('./temp/'.$id.'_f3.mp4') == true)
            echo('<script>$("#LOADING_BAR").width("25%");$("#LOADING_BAR")[0].ariaValueNow = 25; $("#LOADING_BAR")[0].innerText = "25%"</script>');
        else
        {
            ...
            $str = bin2hex($o);
            echo('<script>$ ("#ERRORPHP").click(function() { window.location = "error.php?e=6&r='.$r.'&o='.$str.'"; }); </script>');
            echo('<script>$("#ERRORPHP").trigger("click");</script>'); 
            
            die();
        }
        
        echo('<script>$("#LOADPHP_03").trigger("click");</script>'); //Próxima etapa
        die();
    }
?>
